...
def quote_sm_kit(part_list, part_quantity, components, sch_row, rate, size, sys_type, is_sp_case_1):
    """
    ...
    :param list part_list: already quoted parts for this Package (should be empty for non-stacked kits)
    :param list part_quantity: already quoted quantities for this Package (should be empty for non-stacked kits)
    :param list components: smart kit code (drawing name) split by hyphens
    :param dict sch_row: a row from the smart schedule in json form
    :param float rate: the rate for the kit
    :param str size: the system size for the kit
    :param str sys_type: the connection type for the kit
    ...
    for char in components[0]:
        ...
        if char in pass_chars:
            i += 1
            continue
        # if ss marked on schedule, send to ss_component() function
        elif is_sp_case_1:
        ...
        if components[0].startswith('2'):
            pn, f_size, f_type = type_two_kits(components, pn, size, sys_type, sch_row['conn_size'],
                                               sch_row['conn_type'], sch_row['control_size_type'], rate,
                                               i, f_size, f_type)
        # parse through type 3 kits
        elif components[0].startswith('3'):
            pn, f_size, f_type = type_three_kits(components, pn, size, sys_type, sch_row['conn_size'],
                                                 sch_row['conn_type'], sch_row['control_size_type'], rate,
                                                 i, f_size, f_type)
        # parse through kits with no controls
        else:
            pn, f_size, f_type = no_control_kits(len(components), components, pn, size, sys_type,
...
def quote_large_kit(part_list, part_quantity, components, rate, size):
    """
    Loops through characters in the first half of the smart kit code to identify which components are required. Sends
